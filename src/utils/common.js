...
export default {
     ...
     * 保存资源
     */
    saveResource(type, path, size, mimeType, originalName, storeType, isAdmin = false) {
        let resource = {
            type: type,
            path: path,
            size: size,
        ...
        http.post(constant.baseURL + "/resource/saveResource", resource, isAdmin)
            .catch((error) => {
                ElMessage({
    ...
    async checkLink(url) {
        try {
            ...
            return response.ok
                ? { status: true, message: '地址可访问!' }
                : { status: false, message: `地址不可访问! (状态码: ${response.status})` };
        } catch {
            return { status: false, message: '链接无效或不可访问' };
        }
    },
