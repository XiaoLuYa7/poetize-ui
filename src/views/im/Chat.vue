<template>
    <div>
        ...
        <div class="msg-container">
            <template v-if="!common.isEmpty(currentChatFriendId)">
                <div v-for="(item, index) in imMessages[currentChatFriendId]" :class="[
                    'msg-one',
                    item.fromId === store.state.currentUser.id
                        ? 'message-right'
                        : 'message-left',
                ]" :key="index">
                    <!-- 头像 -->
                    <div>
                        <el-avatar shape="square" object-fit="cover" :size="40" lazy style="cursor: pointer"
                            @click="openFriendCircle(item.fromId, item.avatar)" :src="item.avatar" />
                    </div>
            ...
            <template v-else-if="!common.isEmpty(currentChatGroupId)">
                <div v-for="(item, index) in groupMessages[currentChatGroupId]" :class="[
                    'msg-one',
                    item.fromId === store.state.currentUser.id
                        ? 'message-right'
                        : 'message-left',
                    ...
                    <!-- 头像 -->
                    <div>
                        <el-avatar object-fit="cover" shape="square" :size="40" lazy style="cursor: pointer"
                            @click="openFriendCircle(item.fromId, item.avatar, item.username)" :src="item.avatar" />
                    </div>

                    <!-- 文本消息 -->
                    <div class="message" v-html="item.content" :style="{
                        background:
                            item.fromId !== store.state.currentUser.id
                                ? constant.tree_hole_color[
                                item.fromId % constant.tree_hole_color.length
                                ]
                                : '',
                    }"></div>
        ...
        <div style="height: 180px">
            <!-- 功能栏 -->
            <div style="padding: 10px 15px; display: flex; height: 45px">
                ...
                <div class="myEmoji" @click="sendPicture()">
                    <svg viewBox="0 0 1024 1024" width="25" height="25">
                        <path
                            ...
                            fill="#E0995E"></path>
                        <path d="M736.34 428.748V316.902H288.958v447.382H736.34z" fill="#231F20"></path>
                        <path
                ...
                <!-- 表情包 -->
                <el-popover placement="top-start" transition="el-zoom-in-center" :visible="showPopoverImage"
                    ...
                    <div class="image-list">
                        <el-image v-for="(item, index) in imageList" :src="item" @click="sendImage(item)" :key="index"
                            style="width:80px;height:75px" v-click-outside="onClickOutside"></el-image>
                    </div>
                </el-popover>

                <!-- 舔狗日记 -->
                <el-popconfirm confirm-button-text="确认" cancel-button-text="取消" :hide-icon="true"
                    title="💕&nbsp;投递舔狗日记？" placement="top" @confirm="sendPoetry(3)">
                    <template #reference>
                        <div class="myEmoji">
                            <svg viewBox="0 0 1024 1024" width="25" height="25">
                                <path
                ...
                <!-- 鸡汤来咯 -->
                <el-popconfirm confirm-button-text="确认" cancel-button-text="取消" :hide-icon="true" title="💕&nbsp;鸡汤来咯！"
                    placement="top" @confirm="sendPoetry(4)">
                    <template #reference>
                        <div class="myEmoji">
                            <svg viewBox="0 0 1024 1024" width="25" height="25">
                                <path
                                ...
                                <path
                                    ...
                                    fill="#7F0518"></path>
                                <path d="M697 579.1m-54.5 0a54.5 54.5 0 1 0 109 0 54.5 54.5 0 1 0-109 0Z"
                                    fill="#7F0518"></path>
                                <path d="M683.3 568m-25.1 0a25.1 25.1 0 1 0 50.2 0 25.1 25.1 0 1 0-50.2 0Z"
                                    fill="#FFFFFF"></path>
                                <path d="M715.4 604.6m-11.5 0a11.5 11.5 0 1 0 23 0 11.5 11.5 0 1 0-23 0Z"
                                    fill="#FFFFFF"></path>
                                <path d="M329 579.1m-54.5 0a54.5 54.5 0 1 0 109 0 54.5 54.5 0 1 0-109 0Z"
                                    fill="#7F0518"></path>
                                <path d="M315.3 568m-25.1 0a25.1 25.1 0 1 0 50.2 0 25.1 25.1 0 1 0-50.2 0Z"
                                    fill="#FFFFFF"></path>
                                <path d="M347.5 604.6m-11.5 0a11.5 11.5 0 1 0 23 0 11.5 11.5 0 1 0-23 0Z"
                                    fill="#FFFFFF"></path>
                                <path
                ...
                <!-- 社会语录 -->
                <el-popconfirm confirm-button-text="确认" cancel-button-text="取消" :hide-icon="true"
                    title="💕&nbsp;传教社会语录？" placement="top" @confirm="sendPoetry(5)">
                    <template #reference>
                        <div class="myEmoji">
                            <svg viewBox="0 0 1024 1024" width="25" height="25">
                                <path
                ...
                <!-- 诗词炮弹 -->
                <el-popconfirm confirm-button-text="确认" cancel-button-text="取消" :hide-icon="true"
                    title="💕&nbsp;发射诗词炮弹？" placement="top" @confirm="sendPoetry(1)">
                    <template #reference>
                        <div class="myEmoji">
                            <svg viewBox="0 0 1024 1024" width="25" height="25">
                                <path
            ...
            <!-- 输入框 -->
            <textarea @keydown="send($event)" v-model="msg" maxlength="1000" spellcheck="false" class="message-content"></textarea>
            <!-- 发送 -->
            <div class="message-send">
                <span style="color: var(--greyFont); margin-right: 15px; font-size: 12px">Ctrl+Enter：换行 |
                    Enter：发送</span>
                <el-button @click="doSend()" type="primary"> 发送 </el-button>
            </div>
        </div>

        <!-- 聊天图片弹出框 -->
        <el-dialog draggable title="上传图片" v-model="showPictureDialog" width="30%" :append-to-body="true"
...
const emit = defineEmits(['openFriendCircle', 'sendMsg'])
...
const doSend = () => {
    ...
    if (!common.isEmpty(props.currentChatFriendId)) {
        ...
        let success = false
        emit('sendMsg', JSON.stringify(message), (val) => {
            success = val
        })
        if (success) {
            data.msg = ''
...
const { msg, showPictureDialog, picturePrefix, showPopoverImage } = toRefs(data)
